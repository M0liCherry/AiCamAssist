...
export const policies: Record<PolicyKey, Policy> = {
  privacy: {
    ...
    summary: "What NitroAI stores on your computer, what can leave it, and the choices you control.",
    sections: [
      { heading: "1. Local-first by design", paragraphs: [`${APP_NAME} is a desktop application. Your subjects, chapters, notes, extracted text, retrieval chunks and embeddings, chat history, generated podcasts, flashcard decks, quiz attempts, transcripts, and settings are stored in an embedded database inside your Windows user profile (by default %AppData%\\NitroAI). There is no NitroAI account, no cloud copy, and no server operated by the publisher that receives your content.`] },
      ...
      { heading: "7. Children", paragraphs: ["NitroAI is not directed at children under 13 (or the minimum age of digital consent in your region). Schools deploying the app should review applicable student-privacy rules (e.g. FERPA/COPPA) and the cloud-provider option before use."] },
  ...
  terms: {
    ...
    summary: "Acceptable use, AI limitations, and liability terms for the NitroAI desktop software.",
    sections: [
      { heading: "1. The software", paragraphs: [`These terms cover the ${APP_NAME} desktop application (version ${APP_VERSION}) provided by ${PUBLISHER.name}. By installing or using it you agree to these terms and to the License, Refunds & Support policy.`] },
      ...
      { heading: "8. Termination and governing law", paragraphs: ["You may stop using the software at any time by uninstalling it and deleting your data folder. Governing law and venue are those stated in the License, Refunds & Support policy for the edition you obtained; if none is stated, the mandatory consumer law of your place of residence applies."] },
    ],
  },
  telemetry: {
    title: "Telemetry & Diagnostics Policy",
    summary: "NitroAI ships with zero telemetry. This page documents exactly what the optional diagnostics switch does.",
    sections: [
      { heading: "1. No cookies, no analytics", paragraphs: ["The desktop app renders locally and uses no advertising or analytics cookies, no third-party scripts, and no tracking SDKs. Interface preferences (theme, consent timestamps) are stored in the local database, not in cookies."] },
      ...
      { heading: "3. Crash reporting", paragraphs: ["The Electron crash reporter is not enabled. A crash produces no network traffic. Should a future version add opt-in crash upload, it will be off by default, described here, and require a separate explicit consent."] },
  ...
  license: {
    ...
    sections: [
      { heading: "1. Licence grant", paragraphs: [`${PUBLISHER.name} grants you a personal, non-exclusive, non-transferable licence to install and use ${APP_NAME} on Windows devices you control for personal study and note-taking. You may not redistribute, sell, sublicense, or reverse engineer the application except to the extent permitted by law or by the open-source licences of its components (see Open-source licenses).`] },
      { heading: "2. Price and purchases", paragraphs: ["This version of NitroAI contains no in-app purchases, subscriptions, trials, or licence keys. Any fees charged by third-party AI providers or model hosts are governed solely by those providers. If a paid edition is offered in the future, the price, billing interval, trial length, renewal rules, and refund terms will be shown before purchase and documented here."] },
      { heading: "3. Refunds", paragraphs: ["Because nothing is charged for this edition, no refund can arise from it. If you obtained NitroAI through a store or reseller, that store's refund policy applies to the transaction. Statutory withdrawal and refund rights in your jurisdiction are never limited by this policy."] },
      { heading: "4. Support", paragraphs: [`Support is provided on a best-effort basis by email at ${PUBLISHER.supportEmail}. Include the app version (shown in Settings → About), your Windows version, the AI backend in use, and — if you enabled it — the local diagnostics log. There is no guaranteed response time or uptime commitment for a locally installed application.`] },
  ...
  accessibility: {
    ...
    summary: "How NitroAI approaches WCAG 2.1 AA and where the current limitations are.",
    sections: [
      { heading: "1. Commitment", paragraphs: ["NitroAI is built to meet WCAG 2.1 Level AA: semantic landmarks and headings, labelled controls, visible focus indicators, high-contrast colour tokens in both themes, reflow down to narrow window sizes, and reduced-motion support."] },
  ...
  licenses: {
    ...
    summary: "Third-party software, icons, fonts, and model weights used by NitroAI, with their licences.",
  ...
  return (
    <main className="legal-page">
      <header className="legal-topbar">
        <a href="/" className="legal-brand"><span><Sparkles size={16} aria-hidden="true" /></span>{APP_NAME}</a>
