...
export async function POST(request: NextRequest) {
  try {
    const sampleRate = Number(request.headers.get("x-verity-sample-rate") ?? request.headers.get("x-verity-sample-rate") ?? 0);
    if (sampleRate !== 16000) throw new HttpError("Audio must be resampled to 16 kHz mono before transcription.");
    const language = (request.headers.get("x-verity-language") ?? request.headers.get("x-verity-language") ?? "").trim().toLowerCase().slice(0, 8) || undefined;
