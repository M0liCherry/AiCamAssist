...
import { ArrowDown, ArrowRight, ArrowUp, AudioLines, BookOpen, ChevronDown, ChevronRight, Download, FileText, FolderPlus, FolderTree, Globe, Layers, Pencil, Plus, Search, Sparkles, Trash2, Video } from "lucide-react";
                ...
                return (
                  <li className="tree-subject" key={subject.id}>
                    <div className={`tree-row ${subjectActive ? "active" : ""}`}>
                      ...
                      <button type="button" className="tree-label" onClick={() => onScope({ scopeType: "subject", scopeId: subject.id })} aria-current={subjectActive ? "true" : undefined}><BookOpen size={15} aria-hidden="true" /><span>{subject.name}</span><small>{subject.noteCount}</small></button>
                          ...
                          return (
                            <li key={chapter.id}>
                              <div className={`tree-row tree-row--chapter ${active ? "active" : ""}`}>
                                <button type="button" className="tree-label" onClick={() => onScope({ scopeType: "chapter", scopeId: chapter.id })} aria-current={active ? "true" : undefined}><Layers size={14} aria-hidden="true" /><span>{chapter.name}</span><small>{chapter.noteCount}</small></button>
