...
import { ArrowLeft, Check, CheckCircle2, ChevronLeft, ChevronRight, Circle, Gauge, Headphones, ListChecks, Pause, Play, RefreshCw, RotateCcw, SkipBack, SkipForward, Sparkles, SquareStack, WandSparkles, XCircle } from "lucide-react";
...
export function PodcastsView(props: StudyProps) {
  ...
  const speak = useCallback(
    ...
    [script, supported, voices, hostVoice, guestVoice, rate],
  );
        ...
        <section className="podcast-layout" aria-label="Podcast player and transcript">
          <div className="podcast-player" tabIndex={0} onKeyDown={onPlayerKey} aria-label="Podcast player. Space plays or pauses, arrow keys change turn.">
            ...
            <div className="player-controls">
              ...
              <label className="rate-select"><span className="sr-only">Playback speed</span><select value={rate} onChange={(event) => setRate(Number(event.target.value))}>{[0.85, 1, 1.15, 1.3].map((value) => <option value={value} key={value}>{value}×</option>)}</select></label>
            </div>
            {voices.length > 0 && (
              <div className="voice-controls">
                <label><span>Host voice</span><select value={hostVoice} onChange={(event) => setHostVoice(event.target.value)}>{voices.map((voice) => <option value={voice.name} key={voice.name}>{voice.name} ({voice.lang})</option>)}</select></label>
                <label><span>Guest voice</span><select value={guestVoice} onChange={(event) => setGuestVoice(event.target.value)}>{voices.map((voice) => <option value={voice.name} key={voice.name}>{voice.name} ({voice.lang})</option>)}</select></label>
              </div>
            )}
          </div>
...
export function FlashcardsView(props: StudyProps) {
  ...
  return (
    <StudyShell props={props} icon={<SquareStack size={25} />} eyebrow="Spaced repetition" title="Make knowledge stick" copy="Cards are synthesized from the notes in the selected scope and scheduled with a spaced-repetition interval." ariaId="flashcards-title">
      <div className="generator-controls">
        <label className="count-select"><span>Cards</span><select value={count} onChange={(event) => setCount(Number(event.target.value))}>{[8, 12, 20].map((n) => <option value={n} key={n}>{n}</option>)}</select></label>
        <button type="button" className="primary-button" onClick={regenerate} disabled={generating || !scope}>{cards.length ? <RefreshCw size={17} aria-hidden="true" /> : <WandSparkles size={17} aria-hidden="true" />}{cards.length ? "Regenerate deck" : "Generate flashcards"}</button>
      </div>
      <GenerationNotice asset={data?.asset ?? null} generating={generating} providerName={providerName} label="flashcards" />
      <AiErrorAlert error={error} onRetry={retry} onConfigure={onConfigureAi} />

      {loading ? <p className="gen-status"><Spinner label="Loading saved deck…" /></p> : !card ? (
        ...
        <section className="flashcard-workspace">
          ...
          <div className="flashcard-stage">
            <button type="button" className={`study-card ${revealed ? "revealed" : ""}`} onClick={() => setRevealed((value) => !value)} onKeyDown={onCardKey} aria-pressed={revealed} aria-label={`${revealed ? "Answer" : "Question"}: ${revealed ? card.answer : card.question}. Press Space or Enter to flip, arrow keys to change card, 1 2 3 to grade.`}>
              <span className="card-face card-front"><small>Question</small><strong>{card.question}</strong><span><Circle size={13} aria-hidden="true" /> Click or press Space to reveal the answer</span></span>
              <span className="card-face card-back"><small>Answer</small><strong>{card.answer}</strong><span><CheckCircle2 size={14} aria-hidden="true" /> Grade your recall below</span></span>
            </button>
          </div>
          <div className="deck-navigation">
            <button type="button" className="icon-button" onClick={() => move(-1)} aria-label="Previous card"><ChevronLeft size={21} aria-hidden="true" /></button>
            {revealed ? (
            ...
            ) : <span className="reveal-hint">Reveal the answer to grade this card</span>}
...
export function QuizzesView(props: StudyProps) {
  ...
  }, [answered, questions.length, submitted, data, answers, score, topics, setData, notify]);
